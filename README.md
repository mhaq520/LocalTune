...
- Apple System 浅色 UI：浅色玻璃材质、柔和渐变背景、圆角卡片和悬浮播放栏统一视觉语言，细滚动条与 reduced-motion 也一并适配
...
- Apple Music 风格 UI 重构：深色玻璃材质、柔和渐变背景、圆角卡片和悬浮播放器统一为更克制的媒体库风格
- 首页与详情页统一了标题栏、卡片和列表的层次，提升封面展示和播放控制的可读性
- 设置页改成 macOS 偏好面板式布局，表单分组与按钮状态更清晰
- 前端静态资源版本号已更新，避免安装新版本后继续缓存旧样式

### v0.1.5

- 浅色 Apple System 风格 UI 重构：全局切换为浅色玻璃材质、暖灰背景和更克制的蓝色强调色
- 首页、详情页和设置页统一为浅色卡片、浅色标题栏与轻阴影层次
- 前端静态资源版本号已更新，避免安装新版本后继续缓存旧样式
